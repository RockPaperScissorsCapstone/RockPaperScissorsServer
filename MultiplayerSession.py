...
class MultiplayerSession(threading.Thread):
    ...
    def startMultiplayerSession(self, conn1, conn2):
        ...
        if conn1wins == 2:
            ...
            dbm = DBManager()
            winnerScore = dbm.getScore([conn1ID])
            dbm = DBManager()
            loserScore = dbm.getScore([conn2ID])
            dbm = DBManager()
            dbm.updateScore([conn1ID, winnerScore, conn2ID, loserScore])
            ...
            conn2.sendall("2".encode('ascii'))
            dbm = DBManager()
            winnerScore = dbm.getScore([conn2ID])
            dbm = DBManager()
            loserScore = dbm.getScore([conn1ID])
            dbm = DBManager()
            dbm.updateScore([conn2ID, winnerScore, conn1ID, loserScore])
            self.messenger.addIpAddress(self.player1Address[0])
            self.messenger.addIpAddress(self.player2Address[0])